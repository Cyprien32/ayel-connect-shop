...
const galleryImages = [
  { src: menthPlus, alt: "Menth Plus", title: "Présentation Menth Plus" },
  { src: menthKids, alt: "Menth Kids", title: "Lancement Menth Kids" },
  { src: baumeKineKids, alt: "Baume Kiné Kids", title: "Atelier Baume Kiné Kids" },
  { src: huileKine, alt: "Huile Kiné", title: "Formation Huile Kiné" },
  { src: gelKine, alt: "Gel Kiné", title: "Démonstration Gel Kiné" },
  { src: menthKidsJar, alt: "Menth Kids Jar", title: "Exposition Menth Kids" },
];
      ...
      <section className="relative py-12 md:py-20 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `linear-gradient(to bottom, hsl(var(--primary) / 0.85), hsl(var(--primary) / 0.9)), url(${productsGroup})` }}>
        <div className="container text-center space-y-4">
          <h1 className="text-4xl font-bold tracking-tight sm:text-5xl text-white">
            {t.events.title}{" "}
            <span className="text-accent">{t.events.titleHighlight}</span>
          </h1>
          <p className="text-lg text-white/80 max-w-2xl mx-auto">{t.events.subtitle}</p>
        </div>
      </section>
      ...
      <section className="py-12">
        <div className="container">
          ...
          <p className="text-center text-muted-foreground mt-8">
            {language === 'fr' ? 'Plus de photos à venir lors de nos prochains événements !' : 'More photos coming from our upcoming events!'}
          </p>
        </div>
      </section>
